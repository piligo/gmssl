...
func Listen(network, laddr string, config *Config) (net.Listener, error) {
	slog.Debug("Certificates num->", len(config.Certificates))
...
func DialWithDialer(dialer *net.Dialer, network, addr string, config *Config) (*Conn, error) {
	...
	slog.Debug("start ->", network+"@"+addr, " Timeout->", dialer.Timeout)
	...
	if err != nil {
		slog.Error("Dial network err->", err)
		return nil, err
	}
	...
	if timeout == 0 {
		...
		slog.Debug("Timeout is 0 ")
		err = conn.Handshake()
	} else {
		//否则设置超时阻塞到超时的channel返回
		go func() {
			errChannel <- conn.Handshake()
			slog.Warn("conn.Handshake is End")
		}()
		err = <-errChannel
	}
	if err != nil {
		rawConn.Close()
		slog.Debug("conn.Handshake Err->", err)
		return nil, err
	}
	slog.Debug("conn.Handshake Success Ok")
	return conn, nil
}
