...
func (c *Conn) serverHandshake() error {
	...
	slog.Debug(">>------------serverHandshake ------------<< start  ")
	defer slog.Debug(">>------------serverHandshake ------------<< end  \n")
...
func (hs *serverHandshakeState) readClientHello() (isResume bool, err error) {
	slog.Debug(">>-----------------readClientHello start-----------------<<")
	defer slog.Debug(">>-----------------readClientHello end-----------------<<")
	...
	slog.Debug("hs.clientHello.supportedCurves->", hs.clientHello.supportedCurves)
	slog.Debug("hs.clientHello.preferredCurves->", preferredCurves)
Curves:
	...
	if priv, ok := hs.cert.PrivateKey.(crypto.Signer); ok {
		...
		*/
		case *sm2.PublicKey:
			slog.Debug("PrivateKey Certificate Signer sm2 PublicKey ")
			hs.ecdsaOk = true
			hs.sm2Ok = true
		default:
			c.sendAlert(alertInternalError)
	...
	if priv, ok := hs.encert.PrivateKey.(crypto.Decrypter); ok {
		switch priv.Public().(type) {
		case *sm2.PublicKey:
			slog.Debug("PrivateKey Certificate Decrypter sm2 PublicKey ")
	...
	//slog.Debug(" Piligo PreferServerCipherSuites->", c.config.PreferServerCipherSuites)
	...
	//slog.Debug(" Piligo preferenceList->", preferenceList, "supportedList ->", supportedList)
	for _, id := range preferenceList {
		if hs.setCipherSuite(id, supportedList, c.vers) {
			break
		}
	}
	//slog.Debug(" Piligo hs.suite->", hs.suite)
	if hs.suite == nil {
		c.sendAlert(alertHandshakeFailure)
...
func (hs *serverHandshakeState) setCipherSuite(id uint16, supportedCipherSuites []uint16, version uint16) bool {
	...
	slog.Debug("setCipherSuite id->", id, " supportedCipherSuites->", supportedCipherSuites)

	for _, supported := range supportedCipherSuites {
		if id == supported {
			//slog.Debug("piligo setCipherSuite 01->", cipherSuites)
			var candidate *cipherSuite
			for _, s := range cipherSuites {
				//slog.Debug("piligo setCipherSuite 02 s.id->", s.id)
				if s.id == id {
					candidate = s
					break
				}
			}
			//slog.Debug("piligo setCipherSuite 02->")
			if candidate == nil {
				continue
			}
			//slog.Debug("piligo setCipherSuite 03->")
			// Don't select a ciphersuite which we can't
			// support for this client.
			if candidate.flags != suiteGMTLS11 {
				continue
			}
			//slog.Debug("piligo setCipherSuite 04->")
			if version != VersionGMTLS && version < VersionTLS12 && candidate.flags&suiteTLS12 != 0 {
				//slog.Debug("piligo setCipherSuite 05->")
				continue
			}
			//slog.Debug("piligo setCipherSuite 06->")
			hs.suite = candidate
			return true
		}
	}
